...
const View = () => {
    ...
    const buildUserAccessUrl = () => {
        ...
        return `${BASEURL}/api/portal/master-data/master-kolektif?${params.toString()}`;
    };
    ...
    // Frontend filtering based on search query (in case backend doesn't support q)
    const filteredData = useMemo(() => {
        ...
        // Filter the data array based on search query (nama or no_kolektif)
        const filtered = data.data.filter((item: any) => {
            const nama = (item.nama || "").toLowerCase();
            const noKolektif = (item.no_kolektif || "").toLowerCase();
            return nama.includes(query) || noKolektif.includes(query);
        });
