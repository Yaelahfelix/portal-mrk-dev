...
const View = () => {
    ...
    const buildUrl = () => {
        ...
        return `${BASEURL}/api/portal/master-data/master-loket?${params.toString()}`;
    };

    const { data, isLoading, isValidating, error } = useSWR(buildUrl(), fetcher, {
    ...
    const filteredData = useMemo(() => {
        ...
        // Filter the data array based on search query (loket or kodeloket)
        const filtered = data.data.filter((item: any) => {
            const loket = (item.loket || "").toLowerCase();
            const kodeloket = (item.kodeloket || "").toLowerCase();
            return loket.includes(query) || kodeloket.includes(query);
        });
    ...
    const hasValidData =
        filteredData && filteredData.data && Array.isArray(filteredData.data);
    ...
    return (
        <div className="my-10 px-4 lg:px-6 max-w-[95rem] mx-auto w-full flex flex-col gap-4">
            ...
            <TableFunction limit={String(limit)} />
